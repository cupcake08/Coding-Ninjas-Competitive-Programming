...
He gave this responsibility to Aahad. Aahad did an excellent job of organizing the test. 
As a reward, the professor gave him an sequence of numbers to play with. But Aahad likes playing with "interesting" sequence of numbers,
...
Integer k is the number of coins needed to perform the first operation. Integer l is the number of coins needed to perform the second operation.
...
The professor has a sequence with 5 elements. To perform the first operation, they must pay 2 coin and to perform the second operation, they must pay 1 coin. 
The optimal strategy is:
...
int n,k,l,_min=INT_MAX,_max=INT_MIN;
vector<int> v;
int cost_calc(int num){
    int inc=0,dec=0;
    for(int i=0;i<n;i++){
        if(v[i]>num) dec+=(v[i]-num);
        if(v[i]<num) inc+=(num-v[i]);
    }
    if(dec > inc) return INT_MAX;
    return (dec*k)+((inc-dec)*l);
}
int solve(){
    int cost=INT_MAX;
	for(int i=_min;i<=_max;i++){
        cost=min(cost,cost_calc(i));
    }
    return cost; 
}
int main() {
